...
func (s *Server) handleLoadTestStart(w http.ResponseWriter, r *http.Request) {
    ...
    if _, ok := body["rate"]; !ok { body["rate"] = 10000 }
    if _, ok := body["size"]; !ok { body["size"] = 300 }
    if _, ok := body["workers"]; !ok { body["workers"] = 4 }
    name := "Synthetic Load"
    var srcID string
    for _, s2 := range s.pipeline.GetSources() { if s2.Type == "synthetic" { srcID = s2.ID; break } }
    if srcID == "" {
        created, err := s.pipeline.CreateSource(name, "synthetic", body)
        if err != nil { http.Error(w, err.Error(), http.StatusInternalServerError); return }
        if m, ok := created.(*memSource); ok { srcID = m.ID } else if mm, ok := created.(map[string]interface{}); ok { if id, _ := mm["ID"].(string); id != "" { srcID = id } }
    } else {
        _ = s.pipeline.UpdateSource(srcID, name, body)
    }
    if err := s.pipeline.StartSource(srcID); err != nil { http.Error(w, err.Error(), http.StatusInternalServerError); return }
...
func (s *Server) handleLoadTestStop(w http.ResponseWriter, r *http.Request) {
    var srcID string
    for _, s2 := range s.pipeline.GetSources() { if s2.Type == "synthetic" { srcID = s2.ID; break } }
    if srcID == "" { http.Error(w, "no synthetic source", http.StatusNotFound); return }
    if err := s.pipeline.StopSource(srcID); err != nil { http.Error(w, err.Error(), http.StatusInternalServerError); return }
...
func (s *Server) handleLoadTestStatus(w http.ResponseWriter, r *http.Request) {
    ...
    var produced uint64
    for _, s2 := range s.pipeline.GetSources() { if s2.Type == "synthetic" { srcID = s2.ID; cfg = s2.Config; break } }
    if srcID == "" { http.Error(w, "no synthetic source", http.StatusNotFound); return }
    ...
    if me, ok := s.pipeline.(*memoryEngine); ok {
        ...
        me.mu.RLock()
        for _, ms := range me.sources { if ms.ID == srcID { produced = ms.produced.Load(); break } }
        me.mu.RUnlock()
    }
    ...
    if prevTS > 0 && produced >= prevC {
        ...
        if dt > 0 { eps = float64(produced-prevC) / dt }
    }
    w.Header().Set("Content-Type", "application/json")
