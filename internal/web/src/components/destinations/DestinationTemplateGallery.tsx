...
import { destinationTemplates, DestinationTemplate } from './destinationTemplates'

type DestinationTemplateGalleryProps = {
  onSelectTemplate: (template: DestinationTemplate) => void
}

export default function DestinationTemplateGallery({ onSelectTemplate }: DestinationTemplateGalleryProps) {
  const [searchQuery, setSearchQuery] = useState('')
  const [categoryFilter, setCategoryFilter] = useState<string>('all')
  const [selectedId, setSelectedId] = useState<string | null>(null)

  const handleTemplateSelect = (template: DestinationTemplate) => {
  ...
  // Filter templates
  const filteredTemplates = destinationTemplates.filter(template => {
  ...
  const difficultyColors: Record<DestinationTemplate['difficulty'], 'success' | 'warning' | 'error'> = {
    easy: 'success',
    medium: 'warning',
    advanced: 'error'
  }
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 4, textAlign: 'center' }}>
        ...
        <TextField
          fullWidth
          size="medium"
          placeholder="Search destination templates..."
          value={searchQuery}
                    ...
                    <Stack spacing={2}>
                      ...
                      <Box sx={{ pt: 1, borderTop: 1, borderColor: 'divider' }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5, flexWrap: 'wrap' }}>
                          <Chip
                            label={template.config.type.replace('_', ' ').toUpperCase()}
                            size="small"
                            variant="outlined"
                            sx={{ fontSize: '0.7rem', height: 20 }}
                          />
                          {template.config.config.batchMaxEvents && (
                            <Chip
                              label={`${template.config.config.batchMaxEvents} events/batch`}
                              size="small"
                              variant="outlined"
                              sx={{ fontSize: '0.7rem', height: 20 }}
                            />
                          )}
                          {template.config.config.compression && (
                            <Chip
                              label={template.config.config.compression.toUpperCase()}
                              size="small"
                              variant="outlined"
                              sx={{ fontSize: '0.7rem', height: 20 }}
                            />
      ...
      {filteredTemplates.length === 0 && (
        <Box sx={{ textAlign: 'center', py: 8 }}>
          <Typography variant="h6" color="text.secondary" gutterBottom>
            No destination templates found
          </Typography>
